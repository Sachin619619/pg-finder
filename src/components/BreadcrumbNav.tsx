...
export default function BreadcrumbNav({ items }: BreadcrumbNavProps) {
  return (
    <nav className="flex items-center gap-2 text-xs text-gray-500 mb-4 overflow-x-auto scrollbar-hide">
      <Link href="/" className="hover:text-[#1B1C15] transition-colors shrink-0">Home</Link>
      {items.map((item, i) => (
        <span key={i} className="flex items-center gap-2 shrink-0">
          ...
          {item.href ? (
            <Link href={item.href} className="hover:text-[#1B1C15] transition-colors">{item.label}</Link>
          ) : (
